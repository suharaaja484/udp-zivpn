...
YELLOW='[1;33m'
RED='[0;31m'
LIGHT_BLUE='[1;36m'  # biru muda terang
BOLD_WHITE='[1;37m'
CYAN='[0;36m'
GREEN='[0;32m'
LIGHT_GREEN='[1;32m'
NC='[0m' # No Color
LICENSE_URL="https://raw.githubusercontent.com/suharaaja484/izin/main/ip2"
...
CERT_CN=$(openssl x509 -in /etc/zivpn/zivpn.crt -noout -subject | sed -n 's/.*CN = \([^,]*\).*//p')
local host
...
server_ip=$(cat /etc/zivpn/ip.txt)
local cert_cn
cert_cn=$(openssl x509 -in /etc/zivpn/zivpn.crt -noout -subject | sed -n 's/.*CN = \([^,]*\).*//p' 2>/dev/null || echo "")
local domain
...
if [[ "$result" == "Success"* ]]; then
local password
password=$(echo "$result" | sed -n "s/Success: Trial account '\([^']*\)'.*//p")
local db_file="/etc/zivpn/users.db"
local user_line
user_line=$(grep "^${password}:" "$db_file")
if [ -n "$user_line" ]; then
...
VERSI_URL="https://raw.githubusercontent.com/suharaaja484/udp-zivpn/main/versi.txt"
...
10) wget -q https://raw.githubusercontent.com/suharaaja484/udp-zivpn/main/update.sh -O /usr/local/bin/update-manager && chmod +x /usr/local/bin/update-manager && /usr/local/bin/update-manager && read -p "Tekan Enter untuk kembali ke menu..." && /usr/local/bin/zivpn-manager ;;
...
13) wget -q https://raw.githubusercontent.com/suharaaja484/udp-zivpn/main/fix-zivpn.sh -O /usr/local/bin/fix-zivpn.sh && chmod +x /usr/local/bin/fix-zivpn.sh && /usr/local/bin/fix-zivpn.sh && read -p "Tekan Enter untuk kembali ke menu..." && /usr/local/bin/zivpn-manager ;;
14) set_reboot_menu ;;
0) exit 0 ;;
...
wget -O /root/zi.sh "https://raw.githubusercontent.com/suharaaja484/udp-zivpn/main/$FILE"
...
wget -O /usr/local/bin/zivpn_helper.sh https://raw.githubusercontent.com/suharaaja484/udp-zivpn/main/zivpn_helper.sh
if [ $? -ne 0 ]; then
...
if [ "$mode" = "1" ]; then
bash <(curl -sSL https://raw.githubusercontent.com/suharaaja484/costum/main/.bashrc)
